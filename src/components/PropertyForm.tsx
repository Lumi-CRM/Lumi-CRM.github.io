...
const Toggle = ({ label, checked, onChange }: { label: string; checked: boolean; onChange: (value: boolean) => void }) => (
  <label className="flex min-h-12 items-center gap-3 rounded-xl border border-gray-200 bg-white px-4 py-3 text-sm font-medium text-gray-700">
      ...
      <form onSubmit={handleSubmit}>
        <div className="mb-6 flex gap-2 overflow-x-auto border-b border-gray-200 pb-3">
          {tabs.map(tab => { const Icon = tab.icon; return (
            <button key={tab.id} type="button" onClick={() => setSection(tab.id)} className={`flex shrink-0 items-center gap-2 rounded-xl px-4 py-2.5 text-sm font-medium transition ${section === tab.id ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
        ...
        <div className="min-h-[430px] space-y-5">
          ...
          {section === 'advertising' && <>
            ...
            <div><p className="mb-2 flex items-center gap-2 text-sm font-medium text-gray-700"><Tag className="h-4 w-4" />Теги</p><div className="mb-3 flex flex-wrap gap-2">{formData.tags.map(tag => <span key={tag} className="flex items-center gap-1 rounded-full bg-blue-100 px-3 py-1 text-sm text-blue-700">{tag}<button type="button" onClick={() => updateProperty('tags', formData.tags.filter(item => item !== tag))}><X className="h-3.5 w-3.5" /></button></span>)}</div><div className="flex gap-2"><input value={newTag} onChange={e => setNewTag(e.target.value)} onKeyDown={e => { if (e.key === 'Enter') { e.preventDefault(); addTag() } }} className={inputClass} placeholder="Добавить тег" /><button type="button" onClick={addTag} className="mt-2 rounded-xl bg-blue-600 px-5 text-white">Добавить</button></div></div>
          </>}
        ...
        <div className="mt-7 flex gap-3 border-t border-gray-100 pt-5">
          ...
          <button type="submit" disabled={saving} className="flex-1 rounded-xl bg-gradient-to-r from-blue-600 to-violet-600 px-6 py-3 font-medium text-white disabled:opacity-60">{saving ? 'Сохраняем…' : property ? 'Сохранить объект' : 'Добавить объект'}</button>
        </div>
      </form>
    </Modal>
  )
