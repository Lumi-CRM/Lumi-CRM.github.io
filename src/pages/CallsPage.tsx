...
import { Calendar, Clock, ExternalLink, Filter, Phone, Plus, Search, Trash2 } from 'lucide-react'
...
const CallsPage = () => {
  ...
  const setField = (key: keyof ReturnType<typeof initialForm>, value: string) => setForm(current => ({ ...current, [key]: value }))
  ...
  const save = async (event: React.FormEvent) => {
    ...
    setSaving(false)
    if (insertError) {
      setError('Не удалось сохранить звонок')
      return
    }
  ...
  const field = (key: keyof ReturnType<typeof initialForm>, label: string, type = 'text', placeholder = '') => (
  ...
  return (
    <div className="min-w-0 space-y-6 pb-10">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <button type="button" onClick={() => setIsModalOpen(true)} className="lumi-gradient-button flex w-full items-center justify-center gap-2 rounded-xl px-5 py-3 font-semibold sm:w-auto"><Plus className="h-5 w-5" />Записать звонок</button>
      </div>
      ...
      {loading ? <p className="lumi-muted py-12 text-center">Загружаем журнал…</p> : filtered.length === 0 ? (
        ...
        <div className="grid min-w-0 gap-4 lg:grid-cols-2 2xl:grid-cols-3">
          {filtered.map(call => {
            ...
            return <article key={call.id} className="lumi-panel min-w-0 rounded-2xl border p-5">
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Записать выполненный звонок">
        <form onSubmit={save} className="space-y-6">
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end"><button type="button" onClick={() => setIsModalOpen(false)} className="lumi-control rounded-xl px-5 py-3">Отмена</button><button type="submit" disabled={saving} className="lumi-gradient-button rounded-xl px-6 py-3 font-semibold disabled:opacity-60">{saving ? 'Сохраняем…' : 'Сохранить звонок'}</button></div>
        </form>
      </Modal>
    </div>
  )
