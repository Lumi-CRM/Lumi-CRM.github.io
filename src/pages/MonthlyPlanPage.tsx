...
const MonthlyPlanPage = () => {
  ...
  return <div className="min-w-0 space-y-6 pb-10">
    ...
    {loading ? <p className="lumi-muted py-12 text-center">Загружаем план…</p> : <div data-print-list className="grid gap-4 lg:grid-cols-2">{METRICS.map(([key, label, unit]) => {
      ...
      const progress = calculatePlanProgress(actual[key], target)
      return <section key={key} className="lumi-panel rounded-2xl border p-5"><div className="flex items-start gap-3"><div className="lumi-accent-soft rounded-xl p-2.5"><Target className="h-5 w-5" /></div><div className="min-w-0 flex-1"><h2 className="lumi-text font-semibold">{label}</h2><p className="lumi-muted mt-1 text-sm">Факт: <strong className="lumi-text">{unit === 'money' ? `${actual[key].toLocaleString('ru-RU')} ₽` : actual[key]}</strong> из {unit === 'money' ? `${target.toLocaleString('ru-RU')} ₽` : target}</p></div><input aria-label={`План: ${label}`} type="number" min="0" value={target} onChange={event => changeTarget(key, event.target.value)} className="lumi-control w-28 rounded-xl px-3 py-2 text-right" /></div><div className="lumi-control mt-4 h-2 overflow-hidden rounded-full"><div className="h-full rounded-full bg-[var(--lumi-accent)] transition-all" style={{ width: `${progress.barPercent}%` }} /></div><p className="lumi-muted mt-1 text-right text-xs">{progress.label}</p><div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-5">{weeklyTargets.map((week, index) => <label key={index} className="lumi-muted text-xs">{index + 1} нед.<input aria-label={`${label}, неделя ${index + 1}`} type="number" min="0" value={week[key]} onChange={event => changeWeekTarget(index, key, event.target.value)} className="lumi-control mt-1 w-full rounded-lg px-2 py-2 text-right" /></label>)}</div></section>
    })}</div>}
